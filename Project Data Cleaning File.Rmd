...
Radiant_tower_fb = rep(0,nobs)

for(i in 1:nobs){
  if(length(proj_data[[i]]$objectives) == 0){
    Radiant_tower_fb[i] = NA 
  } else {
    for(j in 1:length(proj_data[[i]]$objectives)){
      if(proj_data[[i]]$objectives[[j]]$subtype == 'CHAT_MESSAGE_TOWER_KILL'){
        Radiant_tower_fb[i] = proj_data[[i]]$objectives[[j]]$team
        break
      } else {
        next
      }
